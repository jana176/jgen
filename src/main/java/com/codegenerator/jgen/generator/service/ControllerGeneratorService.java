...
public class ControllerGeneratorService {
	...
	public void generate(FMDatabaseMetadata databaseMetadata) {
		databaseMetadata.getTables().forEach(table -> generateControllerForModelClass(table));
	}
	
	private void generateControllerForModelClass(FMTable table) {
		...
		try {
			out = generatorService.getAndPrepareWriter(PackageType.CONTROLLER, table.getClassName().concat("Controller"));
			context.clear();
