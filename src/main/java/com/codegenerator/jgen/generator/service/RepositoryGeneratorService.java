...
public class RepositoryGeneratorService {
	...
	private void generateRepositoryForModelClass(FMTable table) {
		final String repoClassName = ClassNamesUtil.toClassName(table.getTableName());
		final String idType = retrieveIdColumnType(table);
		
		Template template = generatorService.retrieveTemplate(PackageType.REPOSITORY);
		imports.add(String.format("generated.model.%s", repoClassName));
		Writer out = null;
		Map<String, Object> context = new HashMap<String, Object>();
		try {
			out = generatorService.getAndPrepareWriter(PackageType.REPOSITORY, ClassNamesUtil.toClassName(table.getTableName()).concat("Repository"));
			context.clear();
			context.put("repoClassName", repoClassName);
