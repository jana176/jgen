...
@Service
public class EnumGeneratorService extends BasicGenerator {
	...
	private void generateEnum(Enumeration enumeration, String packagePath, String packageName) {
		Template template = retrieveTemplate(PackageType.ENUMERATION);
		Writer out = null;
		Map<String, Object> context = new HashMap<String, Object>();
		try {
			out = getAndPrepareWriter(packagePath + File.separator + PackageType.MODEL.toString().toLowerCase()
					+ File.separator + PackageType.ENUMERATION.toString().toLowerCase() + File.separator
					+ enumeration.getEnumType() + ".java");
			context.clear();
